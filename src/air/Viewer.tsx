...
//
// Кадр состоит из двух слоёв решения: СТИЛЬ и ШАБЛОН.
// Стиль (data-anim) — как ведёт себя кадр: скорости входа, камера, переходы.
// Шаблон (data-template) — что в кадре: декорации, палитра, типографика.
// Стиль меняет движение, шаблон меняет мир. Комбинация из трёх стилей и
// трёх шаблонов даёт девять разных эфиров из одних и тех же сцен.
...
import { normalizeStyle, normalizeTemplate } from '@/lib/air/types';
...
function demoState(): AirState {
  ...
  return {
    ...
    theme: { accent: '#ff6fb1', style: 'sport', template: 'cup' },
  };
}
...
function styleOf(theme: AirTheme | null, id: string): AirStyle {
  return normalizeStyle(theme?.[`style:${id}`] ?? theme?.style);
}

/** Шаблон кадра: декорации и палитра, одни на весь эфир. */
function templateOf(theme: AirTheme | null): AirTemplate {
  return normalizeTemplate(theme?.template);
}

/**
 * Тип траектории камеры — по сцене. Работает только в стиле «Кинематограф»:
 * это не «наезд на всё», а разные планы: у «Хода матча» камера скользит
 * вдоль маппула, у представления — облетает соперников, у врезок — резкий
 * короткий наезд, у финалов — медленный подъём. Скорости тоже свои: короткая
 * сцена летит быстрее, длинная — тянется.
 */
    ...
    setState((prev) => {
      switch (m.kind) {
        ...
        case 'scene':
          ...
          // Тему прикладывает мок показа в браузере: смена стиля или шаблона
          // доходит до открытой страницы без перезагрузки. Настоящий сервер
          // тему в этом сообщении не присылает — она и так лежит в каждом
          // снимке.
 ...
 * посередине сцены видит конечное положение, а не дёргающийся перезапуск. Стиль
 * анимации приходит в теме и ставится сюда атрибутом `data-anim`, шаблон —
 * атрибутом `data-template`: по ним сцены и декорации достраивают свой вход и
 * свой вид (селекторы в `air.css`, `deck.module.css` и в модулях сцен).
 *
 * «Кинематограф» — другой язык кадра: мир с перспективой, летящая камера с
 * траекторией по сцене и глубина у объектов. Камера летит весь кадр, а не
 * только на входе. Спортивный и зрительский оставляют камеру на месте —
 * разница между ними в скоростях и характере сборки, а не в полёте.
 */
function Layer({
  layer,
  top,
  ...
  return (
    <div
      ...
      className={[
        ...
        .join(' ')}
    >
      {/* Декорации шаблона: фон и архитектура кадра. Не содержат данных —
          чистая сцена, на которой сцены раскладывают содержимое. Стиль
          управляет тем, как декорации входят и дышат. */}
